...
public class CosineHemisphereDirections {
    private static final double EPSILON = 1e-6;
    private static final int MU_BIN_COUNT = 10;
    private static final double EXPECTED_MEAN_MU = 2.0 / 3.0;

    private final Vec3 hemisphereCenter;
    private final Vec3 hemisphereNormal;
    ...
    public CosineHemisphereDirections(Vec3 hemisphereCenter, Vec3 hemisphereNormal) {
        this.hemisphereCenter = copyOf(hemisphereCenter);
        this.hemisphereNormal = requireUnitNormal(hemisphereNormal);
        this.uAxis = buildPerpendicularAxis(this.hemisphereNormal);
        this.vAxis = this.hemisphereNormal.cross(uAxis).normalize();
    }
    ...
    private Vec3 toCosineDirection(Vec3 uniformDirection) {
        Vec3 combined = hemisphereNormal.add(uniformDirection);
        ...
        if (combinedLength <= EPSILON) {
            return copyOf(hemisphereNormal);
        }
    ...
    public ValidationResult validateDirections(List<Vec3> directions) {
        ...
        for (Vec3 direction : directions) {
            ...
            double mu = clamp(direction.dot(hemisphereNormal), -1.0, 1.0);
    ...
    public void printReport(List<Vec3> directions, int previewCount) {
        System.out.println("Cosine-weighted directions on hemisphere");
        System.out.printf("C = %s%n", hemisphereCenter);
        System.out.printf("N = %s%n", hemisphereNormal);
    ...
    public void printValidationReport(ValidationResult validation) {
        ...
        System.out.printf("Directions below tangent plane: %d%n", validation.directionsBelowHemisphere);
        System.out.printf("Max length deviation from 1: %.12f%n", validation.maxLengthDeviation);
        System.out.printf("Min dot(dir, N): %.12f%n", validation.minDotWithNormal);
        System.out.printf("Mean mu = dot(dir, N): %.12f%n", validation.meanMu);
        System.out.printf("Mean mu deviation from 2/3: %.12f%n", validation.meanMuDeviationFromExpected);
        System.out.printf("Expected mu-bin counts: %s%n", formatDoubleArray(validation.expectedMuBinCounts));
        System.out.printf("Actual mu-bin counts: %s%n", Arrays.toString(validation.muBinCounts));
